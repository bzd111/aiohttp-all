...
async def api_update_post(request: web.Request) -> web.Response:
    ...
    if fields:
        ...
        await db.execute(f"update posts set {field_names} where id = ?", field_values)
        await db.commit()
...
def get_db_path() -> Path:
    ...
    #     if here == here.parent:
    #         raise RuntimeError("Cann't find root github dir")
    #     here = here.parent
    return here / "db.sqlite3"


async def init_db(app: web.Request) -> AsyncIterator[None]:
